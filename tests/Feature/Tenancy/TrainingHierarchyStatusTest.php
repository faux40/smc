 ...
 *
 * The relation is a single upward pointer (`trainings.superseded_by_id`) —
 * Authorized → Competent → Qualified — transitive all the way up. Resolution
...
class TrainingHierarchyStatusTest extends TestCase
{
    ...
    public function test_a_cycle_in_the_data_terminates_rather_than_hanging(): void
    {
        ...
        $user = User::factory()->for($org, 'organization')->create();
        [$authorized, $competent] = $this->ladder($org);
        DB::table('trainings')->where('id', $competent->id)
            ->update(['superseded_by_id' => $authorized->id]);
        $ta = $this->assign($org, $user, $authorized);
