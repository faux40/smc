...
describe('ClassActionsBar', () => {
    ...
    it('enrolls the selection into a newly created class, then navigates', async () => {
        ...
        expect(routerVisit).toHaveBeenCalledWith(
            expect.objectContaining({ url: '/classes/c1' }),
        );
    });
    ...
    it('disables "add to existing" when no scheduled class includes the training', async () => {
        ...
        await flushPromises();
        expect(
            wrapper.find('[data-testid="add-to-class"]').attributes('disabled'),
        ).toBeDefined();
    });

    it('hides "add to existing class" when no single training is given', () => {
        const wrapper = mountBar({ addTrainingId: undefined });
        expect(wrapper.find('[data-testid="add-to-class"]').exists()).toBe(
            false,
        );
        expect(wrapper.find('[data-testid="assemble-class"]').exists()).toBe(
            true,
        );
    });
    ...
    it('disables both actions when nothing is selected', () => {
        ...
        expect(
            wrapper
                .find('[data-testid="assemble-class"]')
                .attributes('disabled'),
        ).toBeDefined();
    });
});
