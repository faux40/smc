...
export const useOrgSettingsStore = defineStore('orgSettings', () => {
    ...
    const thresholds = computed<TrainingThresholds | null>(() => {
        const org = page.props.org as { training_thresholds: TrainingThresholds | null } | null | undefined;
        return org?.training_thresholds ?? null;
    });

    const dueSoonDays = computed(() => thresholds.value?.due_soon_days ?? DUE_SOON_DAYS_DEFAULT);
    const expiringSoonDays = computed(() => thresholds.value?.expiring_soon_days ?? EXPIRING_SOON_DAYS_DEFAULT);
