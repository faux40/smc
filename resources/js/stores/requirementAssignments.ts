...
export const useRequirementAssignmentsStore = defineStore(
    ...
    () => {
         ...
         */
        const forUser = computed(() => (userId: string): RequirementAssignmentRow[] => {
        ...
        async function destroyByRequirement(
            ...
            const { data } = await axios.delete<{ deleted_ids: string[]; updated_ids: string[] }>(
            ...
            taStore.rows = taStore.rows.filter((r) => !data.deleted_ids.includes(r.id));
            ...
            taStore.rows = taStore.rows.map((ta) => {
                ...
                const hasTargetSource = ta.active_sources.some(
                    (s) => s.sourceable_type === REQUIREMENT_CLASS && s.sourceable_id === requirementId,
                );
