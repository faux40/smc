...
export const useStdFrequenciesStore = defineStore('stdFrequencies', () => {
    ...
    async function load(): Promise<void> {
        ...
        const { data } = await axios.get<StdFrequencyRow[]>(
            '/api/std-frequencies',
            {
    ...
    async function create(
        ...
        library.value = [
            ...library.value,
            { ...data, can_edit: true, can_delete: true },
        ];
    ...
    async function update(
        ...
        library.value = library.value.map((f) =>
            f.id === id
                ? { ...f, name: data.name, repeat_days: data.repeat_days }
                : f,
        );
    }

    async function destroy(id: string): Promise<void> {
        await axios.delete(`/api/std-frequencies/${id}`, {
            headers: defaultHeaders(),
        });
    ...
    function subscribe(orgId: string): void {
        ...
        bind(
            'StdFrequencyCreated',
            (p: { id: string; name: string; repeat_days: number }) => {
        ...
        bind(
            ...
            (p: { id: string; name: string; repeat_days: number }) => {
                library.value = library.value.map((f) =>
                    f.id === p.id
                        ? { ...f, name: p.name, repeat_days: p.repeat_days }
                        : f,
                );
            },
        );
