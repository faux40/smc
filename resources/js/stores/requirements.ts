...
export const useRequirementsStore = defineStore('requirements', () => {
    ...
    async function load(): Promise<void> {
        ...
        const { data } = await axios.get<RequirementRow[]>('/api/requirements', {
    ...
    async function create(payload: RequirementFormPayload): Promise<RequirementRow> {
    ...
    async function update(id: string, payload: RequirementFormPayload): Promise<void> {
        await axios.patch(`/api/requirements/${id}`, payload, { headers: defaultHeaders() });
        loaded.value = false;
        await load();
    }

    async function destroy(id: string): Promise<void> {
        await axios.delete(`/api/requirements/${id}`, { headers: defaultHeaders() });
    ...
    function subscribe(orgId: string): void {
        ...
        bind('RequirementCreated', (p: { id: string; name: string; description: string | null }) => {
        ...
        bind('RequirementUpdated', (p: { id: string; name: string; description: string | null }) => {
            library.value = library.value.map((r) =>
                r.id === p.id ? { ...r, name: p.name, description: p.description } : r,
            );
        });
