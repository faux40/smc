...
Route::middleware(['auth', 'verified', 'throttle:240,1'])->group(function () {
    ...
    // One action, `?dimension=training|requirement|not-required`.
    Route::get('api/compliance/export', [ComplianceController::class, 'export'])->name('compliance.export');
    ...
    Route::get('api/reports/compliance-status', [ReportsController::class, 'complianceStatus'])->name('reports.compliance-status');
    Route::get('api/reports/compliance-status/export', [ReportsController::class, 'complianceStatusExport'])->name('reports.compliance-status-export');
    ...
    Route::get('trainings/{training}', function (Training $training) {
        ...
        return Inertia::render('trainings/Show', [
            'training' => [
                ...
                'satisfied_by_ids' => $training->satisfiers()->pluck('trainings.id')->all(),
            ],
            // TagsField is mounted on the page; hydrate it with the current
